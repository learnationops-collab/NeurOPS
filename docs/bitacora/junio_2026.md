...
- **2 de Junio de 2026**:
  - **Corrección en la Atribución por Agendas y Ventas por Fuente en el Monolito API**:
    - **Resolución Dinámica de Atribución por Instagram (`__init__.py`) [MODIFY]**: Rediseño del cálculo del endpoint `GET /public/financial-sales` en `app/api/public/__init__.py`. Ahora se realiza un cruzado en memoria de todas las ventas del periodo contra la tabla de agendas (`FinancialAgenda`) utilizando el usuario de Instagram normalizado (en minúsculas, sin espacios ni `@`) únicamente para determinar la atribución de citas ("Con Agenda" vs "Sin Agenda") de forma robusta.
    - **Protección y Mantenimiento de la Fuente Original (`__init__.py`) [MODIFY]**: Se descartó la sobreescritura del setter con el cliente de la agenda (dado que en la DB real el campo `agenda.nombre` almacena al Cliente y no al Setter), protegiendo la fuente de marketing original de la venta (`FinancialSale.setter` como Laura, Elias, Workshop, VSL, etc.). Esto soluciona el bug por el cual se mostraban nombres de clientes en el desglose de "Ventas por Fuente (Setter)" del frontend.
    - **Alineación de Setters en la Interfaz (`__init__.py`) [MODIFY]**: Sincronización del listado paginado de ventas devuelto al frontend para inyectar de manera reactiva el setter original limpio ("Sin Setter" o el setter/fuente válido de la venta), garantizando perfecta armonía visual y consistencia de datos entre las tablas detalladas y los gráficos agregados.
    - **Consistencia de Fuentes en el Dashboard de Marketing (`manychat.py`) [MODIFY]**: Corrección del cálculo de desgloses y standings globales de rendimiento por setter (`global_setters`) en `app/api/manychat.py` (endpoint `GET /manychat-webhook/stats/dashboard`). Se restauraron los campos históricos reales (`ag.lead` para la fuente/evento de la agenda y `sale.setter` para la fuente original de la venta) asegurando consistencia histórica absoluta y evitando la presencia de nombres de leads en los standings.
