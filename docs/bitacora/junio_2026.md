...
- **3 de Junio de 2026**:
  ...
  - **Eliminación y Exclusión Local Persistente de Ventas**:
    ...
    - **Sincronización Bidireccional en Caliente (Google Sheets API) [NEW FEATURE]**:
      - **API Backend (`sheets_service.py` y `financial_sales.py`) [MODIFY]**: Se creó el método `update_in_sheets` en `SheetsService` para enviar un POST con la acción `"update"`. Se modificó el endpoint de edición `PUT /public/financial-sales/<int:sale_id>` para que, tras actualizar la base local, propague inmediatamente la modificación (como la corrección de Instagram) a Google Sheets utilizando la `marca_temporal` de la venta.
      - **Apps Script de Google Sheets [DOCUMENTED]**: Se documentó y estructuró el código del webhook en Apps Script para que dé soporte tanto a la inserción (`insert`) como a la actualización (`update`) de celdas específicas en la fila correspondiente, garantizando consistencia permanente y evitando que la resincronización deshaga las atribuciones manuales.
