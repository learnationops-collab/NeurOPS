...
## 18 de Junio 2026
- Se agreg� el endpoint /notifications/read-all para todos los roles (admin, closer, setter, triage).
- Se a�adi� el bot�n 'Marcar Le�das' en la bandeja de leads (MainLayout.jsx).
- Se simplific� la vista de las notificaciones para mostrar solo el contenido y el nombre del lead, haci�ndolo m�s espec�fico y de uso r�pido.
- Se resolvi� el problema de visualizaci�n de conjuntos de anuncios en el Dashboard, modificando el endpoint '/manychat-webhook/stats/dashboard' para incluir todos los anuncios activos (con 0 estad�sticas) y devolviendo sus estados.
- Se implement� la l�gica real de archivado en la base de datos ('status' = 'archived') para Campa�as y Conjuntos de anuncios, reemplazando el guardado local.
- Se agreg� el bot�n para mostrar/ocultar y archivar conjuntos de anuncios en 'AdManagementPage' y 'AdDashboardTab'.

- Se integr� la librer�a 'papaparse' en el frontend para el manejo de archivos CSV.
- Se cre� el componente 'ImportSpendModal' que permite subir el reporte de anuncios desde Meta Ads.
- Se implement� un algoritmo de auto-mapeo que vincula los nombres de los conjuntos de anuncios del CSV con los del sistema NeurOPS.
- Se agreg� el bot�n 'Cargar Inversi�n' en la pesta�a 'PeriodSpendTab' para auto-rellenar los importes gastados, optimizando la carga diaria de inversi�n publicitaria.

- Se modific� el endpoint de actualizaci�n de inversi�n ('PUT /api/ads/period-spend/<id>') para permitir la modificaci�n de la fecha ('start_date' y 'end_date').
- Se integr� un selector de fecha interactivo en el Historial de Inversi�n del panel de Gesti�n de Anuncios, permitiendo corregir f�cilmente la fecha al editar un registro existente.

- Se modific� el comportamiento de la edici�n de fecha en el Historial de Inversi�n para que se aplique al grupo completo de registros (por d�a) en lugar de hacerlo individualmente.
- La opci�n de edici�n de fecha se traslad� al encabezado de cada grupo.


- Se reorganiz� el desglose por closer y por fuente en el Tablero de Agendas (FinancialAgendasPage.jsx) a una vista completamente vertical.
- Se redise�� la celda HoverPercentCell para mostrar el n�mero y el porcentaje de manera simult�nea en posiciones separadas.
- Se a�adi� funcionalidad para colapsar/minimizar las tablas de desglose por closer y por fuente con botones de chevron.

- Se elimin� por completo la funcionalidad de Cartera de Clientes. Se quitaron los imports, pesta�as y rutas correspondientes de useDockNavigation.js, PublicCloserStatsPage.jsx y App.jsx.

- Se corrigi� la comparaci�n de datos en el Dashboard de Setters (MessageTable.jsx) para que los mensajes nuevos o sin registros del periodo anterior se comparen con valores en 0 en lugar de no mostrar datos.

- Se implement� la visualizaci�n de la comparaci�n de periodos anteriores en todas las m�tricas del componente LeadUnifiedKPI.jsx (Leads entrantes, tasa de respuesta, cualificaci�n, no cualificados, respondidos y sin respuesta).

- Se agreg� la comparaci�n del periodo anterior a la Matriz de P�rdida de Pasos y a la Matriz de Tenacidad en Seguimiento. Adem�s, se aument� el tama�o de las fuentes de los t�tulos y cifras para una mejor legibilidad en el dashboard de Setters.

- Se redise�� la visualizaci�n de la gr�fica del embudo en el dashboard de closers (FunnelChart.jsx) reemplazando el gr�fico SVG inestable de Recharts por un componente HTML/CSS personalizado con est�tica glassmorphic y visualizaci�n interactiva de las tasas de conversi�n paso a paso.
