...
- **31 de Julio de 2026**:
  - **Alineación del Pipeline de Confirmaciones con el Prototipo `closer-workspace-v7.html` ([CloserWorkflowPage.jsx](file:///c:/Users/EQUIPO%20DELL/Documents/GitHub/NeurOPS/frontend/src/pages/closer/CloserWorkflowPage.jsx) [MODIFY])**:
    - **Mínimo de caracteres en "¿En qué va el proceso?"**: el textarea de nota del paso de confirmación solo bloqueaba el guardado si estaba vacío. Se alineó con el prototipo (`t.value.trim().length>=10`): ahora `saveConfirmReport` y el botón "Guardar Reporte" exigen al menos 10 caracteres, y además exigen que se haya elegido explícitamente una opción de avance (antes el estado `sessionForm.confirm_status` se pre-rellenaba con el estado actual del lead al abrir la ficha, permitiendo guardar sin elegir nada nuevo; ahora arranca en `null` por sesión).
    - **Celebraciones de hitos en el pipeline**: se agregaron los 3 mensajes de felicitación del prototipo (función `celebrate()`), inexistentes hasta ahora en React: 🎯 al vaciar por completo "Por confirmar" por primera vez en la sesión (`vaciamosPorConfirmar`), ✅ por cada lead confirmado (con conteo ordinal `confirmadosHoy` y barra de progreso), y 🏆 "Pipeline blindado" cuando un confirmado deja en cero tanto "Por confirmar" como "Conversando". Se implementó como modal nuevo (`celebration` state) con la estética violeta/rosa ya usada en el resto del workspace v6/v7.
    - **Etapa "Confirmado" — Reagendar / Descartar / notas**: ya existían las acciones de Reagendar y Descartar para leads ya confirmados; se corrigió que el stepper visual del pipeline (Por confirmar → Conversando → Confirmado) no resaltaba la etapa actual por una comparación case-sensitive (`selectedLead.result === 'Confirmado'` vs. el valor real guardado) y no reconocía `'contactado'` como alias de `conversando` (mismo criterio que ya usa el Kanban). Se normalizó con la misma lógica de bucketing.
    - **Bugfix: "Descartar lead" no hacía nada en el paso de confirmación**: `handleConfirmReason` no tenía ninguna rama para `actionType==='confirm_discard'` (el que usa el botón "Descartar lead" de este paso desde su implementación original), por lo que confirmar el descarte con motivo solo cerraba el diálogo sin llamar a la API ni cambiar nada. Se agregó la rama reutilizando el mismo patrón ya probado de `no_lead` (`POST /closer/appointments/<id>/process` con `status: 'No Lead'`).
    - **Reagendar y Descartar ahora también disponibles en "Por confirmar" / "Conversando"**: el prototipo ofrece "Descartar lead" en cualquier etapa (Reagendar solo aparece cuando el lead está `conversando`, con el aviso explicativo correspondiente si todavía está `por_confirmar`); antes solo estaban disponibles una vez llegado a `confirmado`. Se agregó la sección "Otras acciones" también para las etapas previas, reutilizando el modal de motivo (`ReasonModal`) ya existente.
