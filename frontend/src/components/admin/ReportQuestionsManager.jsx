...
const ReportQuestionsManager = () => {
    ...
    const fetchQuestions = async () => {
        setLoading(true);
        try {
            const res = await api.get(`/admin/db/questions?role=${activeRole}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/admin/db/questions', editingId
                ? { ...formData, id: editingId, role: activeRole }
        ...
        <div className="space-y-8">
            ...
            <div className="flex gap-2 p-1 bg-slate-900/60 rounded-2xl border border-slate-800 w-fit">
                <button
                    ...
                    className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeRole === 'closer'
                ...
                <button
                    onClick={() => setActiveRole('setter')}
                    className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeRole === 'setter'
                        ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20'
                        : 'text-slate-500 hover:text-slate-300'}`}
                >
                    Reporte Setter
            ...
            <div className="bg-slate-900/40 p-10 rounded-[2.5rem] border border-slate-800 space-y-6">
                <h2 className="text-xl font-black text-white italic tracking-tighter uppercase">
                    {editingId ? 'Editar Pregunta' : `Nueva Pregunta (${activeRole})`}
                </h2>
