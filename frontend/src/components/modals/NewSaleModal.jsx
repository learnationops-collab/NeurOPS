...
const NewSaleModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
            <div className="w-full max-w-2xl bg-surface border border-base rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] relative overflow-hidden max-h-[80dvh] flex flex-col">
                ...
                {loading ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center py-12 text-center space-y-6 animate-in zoom-in duration-500">
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
                        {/* Cabecera Fija */}
                        <div className="p-6 md:p-8 border-b border-base/50 flex justify-between items-center bg-surface/50 shrink-0 relative pr-16">
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    <TrendingUp className="text-primary" size={20} />
                                    <h2 className="text-2xl md:text-3xl font-black text-base italic uppercase tracking-tighter">Declarar Venta</h2>
                                </div>
                        ...
                        {/* Cuerpo con Scroll */}
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 lg:p-12 space-y-8 min-h-0">
                            {step === 1 ? (
                                        ...
                                        {!isNewClient ? (
                                            ...
                                            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    ...
                                                    <div className="md:col-span-2 space-y-2 pt-2 border-t border-base/50">
                                    ...
                                    <div className="space-y-5">
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                className="w-full bg-main border border-base rounded-2xl py-3.5 px-5 text-sm outline-none focus:ring-2 focus:ring-primary/50 transition-all font-bold appearance-none cursor-pointer"
                                            >
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-main border border-base rounded-2xl py-3.5 px-5 text-sm outline-none focus:ring-2 focus:ring-primary/50 transition-all font-bold"
                                                placeholder="0.00"
                                            />
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-main border border-base rounded-2xl py-3.5 px-5 text-sm outline-none focus:ring-2 focus:ring-primary/50 transition-all font-bold cursor-pointer"
                                            />
                                        </div>
                                    </div>

                                    {/* Columna Derecha: Tipo, Estado y Automatizaciones */}
                                    <div className="space-y-5">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-muted uppercase tracking-widest ml-1">Tipo de Pago</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                {loadingStatus ? (
                                                    <div className="col-span-2 py-4 flex flex-col items-center justify-center gap-2 bg-main/50 rounded-2xl border border-base border-dashed">
                                                        <Loader2 className="w-4 h-4 animate-spin text-primary" />
                                                        <span className="text-[8px] font-black uppercase text-muted tracking-[0.15em]">Analizando...</span>
                                                    </div>
                                                ) : (
                                                    ['full', 'first_payment', 'down_payment', 'installment', 'renewal'].map(type => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${formData.payment_type === type ? 'bg-primary border-primary text-white shadow-lg' : 'bg-main border-base text-muted hover:text-base'}`}
                                                            >
                                                                {labels[type]}
                                                            </button>
                                                        );
                                        ...
                                        <div className="space-y-2 pt-2 border-t border-base/30">
                                            <label className="text-[10px] font-black text-muted uppercase tracking-widest ml-1">Estado de la Venta</label>
                                            <div className="flex bg-main rounded-xl p-1 border border-base">
                                                {['completed', 'pending'].map(st => (
                                                    <button
                                                        ...
                                                        className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${formData.status === st ? (st === 'completed' ? 'bg-emerald-500 text-white shadow-lg' : 'bg-amber-500 text-white shadow-lg') : 'text-muted hover:text-base'}`}
                                                    >
                        ...
                        {/* Footer Fijo */}
                        <div className="p-6 border-t border-base bg-surface/50 shrink-0 flex flex-col gap-4">
                            {step === 2 && (
                                <div className="space-y-3">
                            ...
                            <div className="flex gap-4">
                                {step === 2 && (
                                    <Button
                                        ...
                                        onClick={() => setStep(1)}
                                        variant="ghost"
                                        className="px-8 py-5 h-16 text-xs font-black tracking-widest"
                                    >
                                        Atrás
                                    </Button>
                                )}
                                ...
                                {step === 1 ? (
                                    isNewClient ? (
                                        <Button
                                            type="button"
                                            onClick={() => {
                                                if (!formData.client_data.name || !formData.client_data.email || !formData.client_data.instagram || !formData.client_data.phone) {
                                                    setError("Por favor completa todos los campos del nuevo cliente.");
                                                    return;
                                                }
                                                setError(null);
                                                setAllowedPaymentTypes(['full', 'first_payment', 'down_payment']);
                                            ...
                                            variant="primary"
                                            className="flex-1 py-5 h-16 text-xs font-black tracking-widest"
                                        >
                                            Siguiente Protocolo
                                        </Button>
                                    ) : (
                                    ...
                                    <Button
                                        ...
                                        loading={submitting}
                                        variant="primary"
                                        className="h-16 flex-1 text-xs font-black tracking-widest"
                                        icon={CheckCircle2}
                                    >
                                        Completar Registro
                                    </Button>
