...
import { LayoutDashboard, FileText, Kanban, BarChart3, PlusCircle } from 'lucide-react';
import SetterReportModal from '../components/SetterReportModal';

const SetterDashboard = () => {
    const [stats, setStats] = useState({ total_agendas: 0, total_openings: 0, weekly_conversion: 0 });
    ...
    const fetchStats = async () => {
        try {
            const res = await api.get('/setter/stats/summary');
            setStats(res.data);
        } catch (err) {
            console.error(err);
        } finally {
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900/40 p-10 rounded-[2.5rem] border border-slate-800 group hover:border-indigo-500/50 transition-all">
                    <p className="text-slate-500 font-black uppercase text-[10px] tracking-widest mb-4">Total Agendas</p>
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-black text-white tracking-tighter">{stats.total_agendas}</span>
                ...
                <div className="bg-slate-900/40 p-10 rounded-[2.5rem] border border-slate-800 group hover:border-emerald-500/50 transition-all">
                    <p className="text-slate-500 font-black uppercase text-[10px] tracking-widest mb-4">Total Aperturas</p>
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-black text-white tracking-tighter">{stats.total_openings}</span>
                ...
                <div className="bg-slate-900/40 p-10 rounded-[2.5rem] border border-slate-800 group hover:border-amber-500/50 transition-all">
                    <p className="text-slate-500 font-black uppercase text-[10px] tracking-widest mb-4">Conversión Semanal</p>
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-black text-white tracking-tighter">{stats.weekly_conversion}%</span>
                        <div className="px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-500 text-[10px] font-black font-sans uppercase">
                            +2.4%
                        </div>
                    </div>
                </div>
