...
const KpiCard = ({ label, value, hero, sm, current, previous, invert, note, warning, metric, irA }) => (
    ...
    >
        ...
        <MetricaClicable irA={irA} destino={destino(metric)} detalle={`${label} · ${value}`}
            className={`block font-black tracking-tighter mt-2 ${sm ? 'text-2xl' : 'text-3xl'} ${warning ? 'text-amber-400' : hero ? 'text-primary' : 'text-base'}`}
            envoltura="p">
            {value}
        </MetricaClicable>
...
const PerformanceKpis = ({ current, previous, deuda, irA }) => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <KpiCard metric="deuda_total_pendiente" label="Deuda total pendiente" value={money(k.deuda_total_pendiente ?? deuda)}
                ...
                warning={k.deuda_vencida > 0} irA={irA} />
        </div>
    );
};
