...
const Stat = ({ label, value, note, color, metric, irA }) => (
    <div className="bg-main/60 border border-base rounded-2xl px-4 py-3.5">
        ...
        <MetricaClicable irA={irA} destino={destino(metric)} envoltura="p"
            detalle={`${label} · ${value}`}
            className="block text-2xl font-black tracking-tighter mt-1"
...
const Bar = ({ label, count, rate, note, color, metric, irA }) => (
    <div>
        <div className="flex justify-between text-[12.5px] font-semibold gap-2">
            ...
            <span className="text-right shrink-0">
                <MetricaClicable irA={irA} destino={destino(metric)} vacio={!count}
                    detalle={`${count} señas · ${label}`}>
                    <b>{count}</b> <span className="text-muted text-[11px]">{rate}%</span>
                </MetricaClicable>
            </span>
        </div>
...
const PerformanceSenas = ({ senas, irA }) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card variant="surface" padding="p-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <Stat metric="senas_count" label="Señas del período" value={senas.count} note={`${money(senas.cash)} cobrados`} color="#F59E0B" irA={irA} />
                    <Stat metric="senas_ticket" label="Seña promedio" value={money(senas.ticket_promedio)} note="monto de reserva" irA={irA} />
                    <Stat metric="senas_por_presentacion" label="Por presentación" value={`${senas.por_presentacion}%`} note={`sobre ${senas.presentaciones} presentaciones`} color="#A78BFA" irA={irA} />
                    <Stat metric="senas_por_llamada" label="Por llamada" value={`${senas.por_llamada}%`} note={`sobre ${senas.asistencias} asistencias`} color="#60A5FA" irA={irA} />
                </div>
                <div className="flex justify-between text-[12px] font-bold border-t border-base mt-4 pt-3">
                    ...
                    <MetricaClicable irA={irA} destino={destino('senas_close_promesa')}
                        detalle={`Cierres más reservas · ${senas.close_rate_promesa}%`}>
                ...
                {senas.seguidas > 0 ? (
                    <>
                        <div className="space-y-3.5">
                            <Bar metric="senas_a_pif" label="Pasaron a pago completo" note="PIF — pagó todo el programa" count={senas.a_pif} rate={senas.a_pif_rate} color="#22C55E" irA={irA} />
                            <Bar metric="senas_a_split" label="Pasaron a Split Pay" note="primer pago, resto en cuotas" count={senas.a_split} rate={senas.a_split_rate} color="#6366F1" irA={irA} />
                            <Bar metric="senas_pendientes" label="Todavía sin pago" note="la reserva no se cerró (o es reciente)" count={senas.pendientes} rate={senas.pendientes_rate} color="#EF4444" irA={irA} />
                        </div>
