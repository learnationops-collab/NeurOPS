                    ...
                    <tbody className="divide-y divide-slate-800/50">
                        {data.sales.map(sale => (
                            <tr key={sale.id} className="hover:bg-slate-800/20 transition-all">
                                ...
                                <td className="px-8 py-5">
                                    <span className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest ${sale.status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                                            sale.status === 'pending' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
                                                'bg-slate-700 text-slate-400'
                                        }`}>
                                        {sale.status === 'completed' ? 'Completado' : sale.status === 'pending' ? 'Pendiente' : sale.status}
                                    </span>
                                </td>
                            </tr>
                        ))}
