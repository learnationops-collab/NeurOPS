...
def marcar_agenda_duplicada(appt_id):
    """Resuelve una agenda duplicada desde la pestana "Agendas" de Mi cartera.

    Las dos guardas (solo la copia sin resultado, y solo si hay una hermana cercana) y el rastro
    de auditoria viven en `closer_agendas_service.marcar_duplicada`, que comparte con el modal
    del lead del dashboard comercial. Aca queda solo QUIEN puede hacerlo, que es lo unico que
    cambia entre las dos rutas: un closer, solo sobre sus propias agendas.
    """
    if current_user.role not in ['closer', 'admin']:
        return jsonify({"message": "Forbidden"}), 403
    ...
    return jsonify({"message": mensaje, "kept_id": conservada}), 200
