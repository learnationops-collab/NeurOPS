...
# Conectar a produccion. Cargar .env de la raiz
if os.path.basename(current_dir) == 'scripts':
...
def ejecutar_importacion():
    with app.app_context():
        ...
        # Intentar cargar desde la raiz o la carpeta scripts
        root_dir = os.path.abspath(os.path.join(current_dir, '..')) if os.path.basename(current_dir) == 'scripts' else current_dir
        FILE_PATH = os.path.join(root_dir, "respuestas_form.md")
        
        if not os.path.exists(FILE_PATH):
            print(f"Error: No se encontró el archivo de respuestas en {FILE_PATH}")
            return
